...
import intersectionOverUnion from '../utils/intersectionOverUnion'
...
export default function ScannerScreen(props: Props) {
  ...
  const pointsData = REA.useDerivedValue(() => {
    ...
    if (detectionResults.value.length > 0) {
      ...
      if (result) {
        ...
        pointsDataValue = `${pointsDataValue + location.points[1].x},${
          location.points[1].y
        } `
        pointsDataValue = `${pointsDataValue + location.points[2].x},${
          location.points[2].y
        } `
        pointsDataValue = `${pointsDataValue + location.points[3].x},${
          location.points[3].y
        }`
        data = pointsDataValue
      }
    }
  ...
  useEffect(() => {
    ;(async () => {
      ...
      let result = await DDN.initLicense(
  ...
  const steady = () => {
    if (
      previousResults.current[0] &&
      previousResults.current[1] &&
      previousResults.current[2]
    ) {
      ...
      console.log(iou2)
      console.log(iou3)
      return iou1 > 0.9 && iou2 > 0.9 && iou3 > 0.9
    }
    return false
  }
